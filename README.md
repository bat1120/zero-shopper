...
- 🔎 **의미 기반 재랭킹 (RAG + Tool)**: 검색 결과를 즉석 임베딩해 사용자의 자연어 의도에 더 맞도록 재정렬 (더미 폴백 경로의 정량 평가는 [검색 평가 문서](docs/search-eval.md) 참고)
...
- **검색 진입점**: `searchProductsAuto()` 가 자격증명 유무로 소스를 자동 선택합니다.
  - **라이브 (네이버 키 있음)**: `searchNaver()` 가 네이버 쇼핑 검색 API를 호출해 실제 상품(제목·가격·이미지·판매처·구매 링크)을 가져옴 → 결과를 **즉석 임베딩**해 쿼리 의도와의 코사인 유사도로 재랭킹(`searchProductsLive`). 카테고리에 묶이지 않아 **거의 모든 상품**을 검색할 수 있음.
...
- **RAG 재랭킹·하이브리드 검색**: 라이브 결과는 즉석 임베딩으로 의도 재랭킹하고, 더미 폴백 경로는 의미 유사도 + 어휘(동의어·태그가중) 점수를 결합해 가중치를 평가 셋으로 튜닝(45:55).
...
- **정량 평가 범위**: Top-1/Recall@3/MRR 평가 하네스는 **더미 카탈로그(폴백 경로)** 기준으로 측정. 라이브 네이버 결과는 정답 라벨이 없어 동일 지표로 평가하지 않음.
